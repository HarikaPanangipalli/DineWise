...
Handles operations related to managing user grocery lists.

Classes:
- GroceryService: Provides methods for grocery list management.
...
class GroceryService:
    ...
    async def store_consolidated_groceries(
        self,
        user_id: str,
    ...
    async def get_last_email_date(self, user_id: str) -> datetime:
        ...
        try:
            latest_grocery = await database.db.groceries.find_one(
                {"user_id": user_id}, sort=[("last_email_date", -1)]
            )
    ...
    async def get_grocery_by_id(self, grocery_id: str, user_id: str) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to fetch grocery list: {str(e)}"
            )

    async def update_grocery_list(
        self, grocery_id: str, user_id: str, new_item: dict
    ) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to update grocery list: {str(e)}"
            )

    async def delete_grocery_item(
        self, grocery_id: str, item_index: int, user_id: str
    ) -> dict:
        ...
        try:
            ...
            updated_grocery = await database.db.groceries.find_one_and_update(
                {"id": grocery_id}, {"$pull": {"items": None}}, return_document=True
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to delete grocery item: {str(e)}"
            )
