...
async def extract_and_store_groceries(
    days: int = 30, current_user: UserResponse = Depends(get_current_user)
):
    ...
    try:
        ...
        last_email_date_extracted = await grocery_service.get_last_email_date(
            current_user.id
        )
        if last_email_date_extracted:
            last_email_date_extracted = last_email_date_extracted.strftime(
                "%Y/%m/%dT%H:%M:%S"
            )
        ...
        if not emails:
            return {"message": "No grocery emails found", "total_emails": 0}
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to process groceries: {str(e)}"
        )
...
async def get_user_grocery_lists(
    ...
    try:
        ...
        return {"status": "success", "total_lists": len(lists), "lists": lists}
    except Exception as e:
